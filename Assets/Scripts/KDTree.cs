// Original from https://gist.github.com/ditzel/194ec800053ce7083b73faa1be9101b0
...
public class KdTree<T> : IEnumerable<T>, IEnumerable where T : StateVector
{
    ...
    protected T _findClosest(Vector3 position, List<T> traversed = null)
    {
        ...
        while (openCur < _open.Length && _open[openCur] != null)
        {
            ...
            if (nodeDist < nearestDist && nodeDist != 0f) // Check against zero to allow passing the position of a current item in the tree
            {
